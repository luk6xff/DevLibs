...
int MPU6050::mpu_init(void)
{
    ...
    /* Reset device. */
    data[0] = BIT_RESET;
    if (!writeByte(st.hw->addr, st.reg->pwr_mgmt_1, data[0] ) )
    {
        return -1;
    }
 ...
 *  @return     0 if successful.
 */
int MPU6050::mpu_get_temperature(int32_t *data, uint32_t *timestamp)
{
    uint8_t tmp[2];
   int16_t raw;
 ...
 *  @return     0 if successful.
 */
int MPU6050::mpu_read_6500_accel_bias(int32_t *accel_bias) {
    uint8_t data[6];
 ...
 *  @return     0 if successful.
 */
int MPU6050::mpu_read_6050_accel_bias(int32_t *accel_bias) {
    uint8_t data[6];
...
int MPU6050::mpu_read_6500_gyro_bias(int32_t *gyro_bias) {
    uint8_t data[6];
 ...
 *  @return     0 if successful.
 */
int MPU6050::mpu_set_accel_bias_6050_reg(const int32_t *accel_bias)
{
    uint8_t data[6] = {0, 0, 0, 0, 0, 0};
 ...
 *  @return     0 if successful.
 */
int MPU6050::mpu_set_accel_bias_6500_reg(const int32_t *accel_bias)
{
    uint8_t data[6] = {0, 0, 0, 0, 0, 0};
    int32_t accel_reg_bias[3] = {0, 0, 0};
    int32_t mask = 0x0001;
